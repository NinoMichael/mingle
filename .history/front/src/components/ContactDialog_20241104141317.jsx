...
const ContactDialog = ({ visible, inputSearchContact, setInputSearchContact, setContactsDialog, activeIndex, setActiveIndex }) => {
    ...
    useEffect(() => {
        if (dialogRef.current) {
            dialogRef.current.classList.toggle("w-[50vw]", !!selectedUser)
            dialogRef.current.classList.toggle("w-[35vw]", !selectedUser)
        }
    }, [selectedUser])
    ...
    return (
        <Dialog
            ...
            className={`bg-blackAccent text-white h-[70vh] p-4 rounded`}
        />
    )
}
