...
const ContactDialog = ({ visible, inputSearchContact, setInputSearchContact, setContactsDialog, activeIndex, setActiveIndex }) => {
                ...
                <div className="overflow-y-auto">
                    {suggestedUser.map((user) => (
                        <div key={user.id} className="flex flex-row justify-between text-white hover:bg-blackPure rounded p-4"
                            onClick={() => setSelectedUser(user)} // Sélectionner l’utilisateur
                        >
    ...
    return (
        <Dialog modal visible={visible} onHide={() => { if (!visible) return; setContactsDialog(false) }} content={({ hide }) => (
            <form>
                ...
                <AnimatePresence>
                    <motion.div
                        ...
                        animate={{ width: selectedUser ? "60%" : "30%" }} // Transition de largeur
                    ...
                    >
                        ...
                        {/* Affichage du profil sélectionné, visible uniquement si `selectedUser` est défini */}
                        {selectedUser && (
                            <motion.div
                                className="ml-4 bg-blackAccent rounded p-4 flex-1"
                                initial={{ opacity: 0 }}
                                animate={{ opacity: 1 }}
                                exit={{ opacity: 0 }}
                            >
                                <h2 className="text-lg text-white font-semibold">{selectedUser.user}</h2>
                                <p className="text-sm text-gray-400">{selectedUser.phoneNumber}</p>
                                <div className="flex flex-row items-center mt-4">
                                    <Button label="Message" className="p-button-rounded p-button-outlined text-white border-white mr-2" />
                                    <Button label="Supprimer" className="p-button-rounded p-button-danger" onClick={() => setSelectedUser(null)} />
                                </div>
                            </motion.div>
                        )}
                    </motion.div>
