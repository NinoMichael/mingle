...
const SideMenuChat = ({ onSelectMessage }) => {
    ...
    const [showContactsDialog, setShowContactsDialog] = useState(false) // État pour le Dialog "Contacts"
    ...
    const menuSideBar = [
        {
            ...
            icon: "pi pi-user",
            menu: "Contacts",
            action: () => setShowContactsDialog(true) // Ouvre le Dialog "Contacts"
        },
        {
            id: 2,
            icon: "pi pi-users",
    ...
    return (
        <section className="fixed float-left -mt-1">
            ...
            <Dialog
                ...
                style={{ width: '400px' }}
                onHide={() => setShowContactsDialog(false)}
            >
                <p>Liste des contacts ici...</p>
                {/* Vous pouvez ajouter ici la liste des contacts ou tout autre contenu */}
            </Dialog>
        </section>
    )
}
