...
const Register = () => {
    ...
    const countryOptionTemplate = (option) => {
        const flagUrl = `https://flagcdn.com/${option.code.toLowerCase()}.png`; // Modifier ici

        return (
            <div className="flex align-items-center">
                <img
                    alt={option.name}
                    src={flagUrl} // Utiliser l'URL du drapeau
                    className={`mr-2`}
                    style={{ width: '18px' }}
                />
                <div className="font-poppins text-xs">{option.name} ({option.dialCode})</div>
            </div>
        );
    };
                ...
                <motion.form className="bg-blackAccent px-12 pt-8 pb-12 rounded shadow" variants={fadeInUp} transition={{ delay: 0.5 }}>
                    ...
                    <div className="p-inputgroup flex-1 mt-8">
                        ...
                        {country && (
                            <img
                                alt={country.name}
                                src={`https://flagcdn.com/${country.code.toLowerCase()}.png`} // Changer ici
                                className={`ml-2`}
                                style={{ width: '18px' }}
                            />
                        )}
