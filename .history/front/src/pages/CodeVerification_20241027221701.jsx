    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.8 }}>
            <div className="flex justify-between mt-8 mx-12">
                ...
                <Dialog modal visible={visible} style={{ width: '50vw' }} onHide={() => { if (!visible) return; setVisible(false) }} className="bg-blackAccent text-white font-poppins text-sm">
                    <div className="flex flex-col justify-center items-center px-auto -pt-8">
                        <img src={emailIcon} alt="Email verification" className="w-64 h-52" />
                        <p className="text-center w-96">Nous avons envoyé un email de vérification à votre adresse e-mail. Veuillez vérifier votre boite e-mail</p>

                        <p className="mt-4 text-xs text-center">Vous n'avez pas encore reçu d'email ? <Link className="text-blueSlate no-underline">Renvoyer</Link></p>
                    </div>
                </Dialog>
            </div>
